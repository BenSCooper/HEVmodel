# HEV metrop-hastings code for model1a.R (basic SEIR model with moderately informative priors)
# Ben Cooper 2016
...
# Model 1a does this with  no watsan effects, camp specific R0s, camp-specific time of first case but estimating 
# gamma and proportion sympomatic which are the same for all three camps.
# Initially do this assuming simple exponential durations in latent and infectious classes
...
# first compile and load the C code for the SEIR model

system("R CMD SHLIB SEIR.c")
dyn.load("SEIR.so")
...
HepEmod.nowatsan <- function(t, x, parms) { 
  ...
  # Function returning a list holding the model equations for model without WATSAN intervention. 
  ...
  with(as.list(c(parms, x)), {
    # beta.max is the  transmission parameter 
...
getLLHepEmod.mh.combined.no.watsan<-function(params, x1,x2,x3, plot=FALSE,verbose=FALSE){
  ...
  with(as.list(c(params, x1,x2,x3)), {
    ...
    out3<-ode(y=x3,times=times3, func="derivs",parms=parameters3,jacfun="jac",dllname="SEIR", initfunc="initmod",nout=1,outnames="Sum")
    ...
    # must have weekly inc greater than observed (otherwise not feasibl)
    if(verbose){
    ...
    disp<-params$disp # dispersion parameter for NegBin distribution (if NA use Poisson instead)
...
LATEST.START.WEEK1<-52 # last week in year one when epidemic could start. (use 52 for camp1, 45 for camp 2 and 49 for camp 3)
LATEST.START.WEEK2<-45
...
wks.to.symptoms<-1  # i.e. delay in weeks from onset of infectiousness to onset of icteric phase of disease
...
#parameters$disp<-1
  current.loglikelihood<-getLLHepEmod.mh.combined.no.watsan(parameters, xstart1,xstart2,xstart3, plot=TRUE)
  ...
  for(i in 1:N){
    ...
    if(proposal.prior.density>0) { # no point running the model if proposal is outside feasbile range as acceptance.prob will be zero
      proposal.loglikelihood<-getLLHepEmod.mh.combined.no.watsan(proposed.parameters, xstart1,xstart2,xstart3, plot=FALSE)
...
write.csv(posterior.samples,file="posterior.samples.model1a.10Mthin100.csv")  
save.image("model1a SEIR inf priors 10M thin 100.RData")




# plot selected runs 
pdf("mod1a SEIR - selected fits from 10M (thin 100) chain.pdf")
...
# plot chains
pdf("mod1a SEIR inf priors NBLL.pdf")
...
# plot correlation
pdf("mod1a SEIR inf prior  NBLL 1correlation.pdf")
par(mfrow=c(3,3),mar=c(4,4,1,1))
rows.to.plot<-seq(1,length(posterior.samples$R01),1)
...
post.means<-data.frame(t(apply(posterior.samples[range,],2,mean)))[1:11]

# 1. Calculate DIC using the Spiegelhalter approach
dhat<-  -2*getLLHepEmod.mh.combined.no.watsan(post.means, xstart1,xstart2,xstart3, plot=FALSE)$LL
...
for(i in runs.to.plot){
  ...
  # below should be same as out1 out2 and out3 calculated using the compiled code (and they are)
...
pdf("Predicted versus observed mod1a SEIR.pdf",height=6,width=5,pointsize=10)
