# HEV metrop-hastings code for model5.R 
#( SEIIR model with moderately informative priors allowing transmisison rate to very depending on whether in the first or second I compartment)
# Ben Cooper 2016
...
# rho.prior.density<-function(x){ dgamma(x, shape=11,rate=1.145)}
 rho.prior.density<-function(x){ dgamma(x, shape=22,rate=1.145)} #SEIIR - twice rate in each I, so half duration . 
...
# with version 1.8 we can do about 20000 runs in one hour - we need to do about 3 million runs in total so really a week long runs are needed.
...
# Below the SEIIR option, if set to tree, causes likelihood to be based on SEIIR model, where time is in two 
...
for(i in 1:N){
  ...
  if(proposal.prior.density>0) { # no point running the model if proposal is outside feasbile range as acceptance.prob will be zero
    ...
    #  updates to  proportion.of.transmission.early.in.infection 
...
#save.image()
save.image("model5 SEIIR different trans from I1 and I2 no watsan MH1.10 inf (uninfbeta) priors 3M thin 100.RData")


# plot selected runs 
pdf("model5 SEIIR - selected fits from .3M (thin 100) chain.pdf")
...
# plot chains
pdf("model5 SEIIR  inf priors NBLL 3M runs thin 100.pdf")
...
# plot correlation
#pdf("model5 SEIIR inf prior  NBLL 3M correlation.pdf")
...
2*mean(365/posterior.samples$rho)
2*quantile( 365/posterior.samples$rho, c(.025,.5,.975))
...
# 6. dispersion parameter of negbin dist
...
pdf("model5 SEIIR  Predicted versus observed 3m thin 100.pdf",height=6,width=5,pointsize=10)
