...
// SEIWR2 differs from SEIWR  only in that it aso keeps track of the number of people who have been infected via the environmental route
...
// of transmission from person equivalents to suseptibles as from infected people to suscR CMD SEIRW2.c eptibles. This should make it identifiable. 
// To compile from R: system("R CMD SHLIB SEIRW2.c")
// To load from R (in Unix): dyn.load("SEIWR2.so")
...
//#include </Library/Frameworks/R.framework/Versions/3.2/PrivateHeaders/R.h> 
#include <R.h> 
...
y[6] is CumInc (cumulative incidence) infected via the environmental route. 
*/
void derivs (int *neq, double *t, double *y, double *ydot, double *yout, int *ip)
{
    ...
    ydot[3] =  rho*y[2]; 
    ydot[4] = beta*y[0]*y[2]/N + beta2*y[0]*y[5]/(N*(1+y[5]));
    ydot[5]=lambda*y[2] - nu*y[5];
    ydot[6]=beta2*y[0]*y[5]/(N*(1+y[5]));
...
void jac(int *neq, double *t, double *y, int *ml, int *mu, double *pd, int *nrowpd, double *yout, int *ip)
{
    ...
    pd[2] =0;
    pd[3] =0;
    pd[4]=beta*y[2]/N + beta2*y[5]/(N*(1+y[5]));
    pd[5]=0;
    pd[6]=beta2*y[5]/(N*(1+y[5]));
    ...
    pd[5*(*nrowpd)+2]=0;  
    pd[5*(*nrowpd)+3]=0;  
    pd[5*(*nrowpd)+4]=-beta2*y[0]*(y[5]/(N*(1+y[5])*(1+y[5])) - 1/(N*(1+y[5]))); 
    pd[5*(*nrowpd)+5]=-nu; 
    pd[5*(*nrowpd)+6]=-beta2*y[0]*(y[5]/(N*(1+y[5])*(1+y[5])) - 1/(N*(1+y[5])));
