# HEV metrop-hastings code for model3.R ( SEEIR model with moderately informative priors allowing transmisison rate to depend on interpolated water per person data)
# Ben Cooper 2016
...
# Model 3 does this with  an SEEIR model,
#(by default with same parameters for E1 and E2 so latent paeriod has erlang distribution)
...
getLLHepEmod.mh.combined.no.watsan<-function(params, x1,x2,x3, plot=FALSE,verbose=FALSE, SEEIR=TRUE){
    ...
    if(SEEIR==FALSE){
     ...
     out3<-ode(y=x3,times=times3, func="derivs",parms=parameters3,jacfun="jac",dllname="SEIR", initfunc="initmod",nout=1,outnames="Sum",maxsteps=1000000)
      ...
      parameters3<-c(
        ...
        gamma2=gamma,
        rho=rho)
      out1<-ode(y=x1,times=times1, func="derivs",parms=parameters1,jacfun="jac",dllname="SEEIR", initfunc="initmod",nout=1,outnames="Sum")
      out2<-ode(y=x2,times=times2, func="derivs",parms=parameters2,jacfun="jac",dllname="SEEIR", initfunc="initmod",nout=1,outnames="Sum")
...
gamma.prior.density<-function(x){ dgamma(x, shape=2,rate=0.0932)} # prior for SEEIR - doubling shape gives double rate of transition through each E, so overall mean latent period should be 34 days
...
#  we can do about 20000 runs in one hour - we need to do about 3 million runs in total so really a week long runs are needed.
...
write.csv(posterior.samples,file="posterior.samples.model3.3Mthin100.csv")  
save.image("model3 SEEIR 3M thin 100.RData")


# plot selected runs 
pdf("mod3 SEEIR WATSAN - selected fits from 3M (thin 100) chain.pdf")

par(mfrow=c(3,2),mar=c(4,4,1,1))
parameters<-posterior.samples[1,]
getLLHepEmod.mh.combined.no.watsan(parameters, xstart1,xstart2,xstart3, plot=TRUE, SEEIR=TRUE)
...
getLLHepEmod.mh.combined.no.watsan(proposed.parameters, xstart1,xstart2,xstart3, plot=TRUE, SEEIR=TRUE)
...
# plot chains
pdf("mod3 SEEIR WATSAN - inf priors NBLL 3M runs thin 100.pdf")
...
# plot correlation
pdf("mod3 NBLL 3M correlation.pdf")
...
dhat<-  -2*getLLHepEmod.mh.combined.no.watsan(post.means, xstart1,xstart2,xstart3, plot=FALSE,verbose=FALSE)$LL
...
2*mean(365/posterior.samples$gamma)
2*quantile( 365/posterior.samples$gamma, c(.025,.5,.975))
...
pdf("Predicted versus observed model3 thin 100.pdf",height=6,width=5,pointsize=10)
